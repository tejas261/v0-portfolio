...
export default function JourneySection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="section relative overflow-hidden bg-gradient-to-br from-background via-secondary/30 to-background"
    >
      ...
      <div ref={containerRef} className="relative h-screen">
        <div className="flex h-full items-center">
          {journeySteps.map((step, index) => (
            <div key={index} className="journey-step flex h-full w-screen shrink-0 items-center justify-center px-20">
              <div className="journey-card group relative">
                {/* Road path visual */}
                <div className="absolute -top-40 left-1/2 h-80 w-1 -translate-x-1/2">
                  <div className={`h-full w-full bg-gradient-to-b ${step.color} opacity-30`} />
                  <div
                ...
                {/* Main card */}
                <div
                  className="relative overflow-hidden rounded-3xl border border-border/50 bg-white/5 p-[3px] shadow-2xl transition-all duration-500 hover:scale-110 hover:shadow-[0_0_60px_rgba(255,255,255,0.4)]"
                  style={{
                    perspective: "1000px",
                  }}
                >
                  <div className="relative h-[400px] w-[500px] overflow-hidden rounded-3xl bg-card/95 backdrop-blur-xl">
                    {/* Emoji background */}
                    ...
                    <div className="relative z-10 flex h-full flex-col justify-between p-10">
                      ...
                      <div className="inline-flex self-start">
                        <span className={`rounded-full ${step.bgColor} px-4 py-2 text-sm font-bold backdrop-blur-sm`}>
                          {step.year}
                        </span>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <h3 className="font-sans text-4xl font-black text-foreground">{step.title}</h3>
                        <p className="text-2xl font-bold text-white">{step.place}</p>
                ...
                {index < journeySteps.length - 1 && (
                  <div className="absolute -right-32 top-1/2 -translate-y-1/2 text-6xl opacity-30">→</div>
                )}
              </div>
            </div>
          ))}
